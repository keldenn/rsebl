...
import { fetchChartDataBsi } from "@/lib/yahoo-finance/fetchChartDataBsi"
...
export default async function BsiChart({
  range,
  interval,
}: StockGraphProps) {
  const chartData = await fetchChartDataBsi( range, interval)

  const [chart] = await Promise.all([chartData])
